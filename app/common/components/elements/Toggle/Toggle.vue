...
.root input:not(:checked) ~ .toggle {
  background-color: var(--ds-color-background-gray-weak);
}
...
.root input:checked ~ .toggle .thumb {
  transform: translateX(var(--ds-dimensions-toggle-thumb));
}
...
.thumb {
  ...
  width: var(--ds-dimensions-toggle-thumb);
  height: var(--ds-dimensions-toggle-thumb);
  border-radius: var(--ds-border-radius-circle);

  background-color: var(--ds-color-background-primary-base);
